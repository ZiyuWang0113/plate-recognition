...
public class PlateUtil {
    ...
    private static SVM svm = null; // 识别图片是否是车牌
    // 简单测试了一下，发现绿牌跟蓝牌分开识别，准确率更高
    private static ANN_MLP ann_blue = null; // 识别蓝牌的字母及数字
    private static ANN_MLP ann_green = null; // 识别绿牌的字母及数字
    private static ANN_MLP ann_cn = null; //识别中文字符
    ...
    public static String charsSegment(Mat inMat, PlateColor color, Boolean debug, String tempPath) {
        ...
        for (int i = 0; i < contours.size(); i++) { // 遍历轮廓
            ...
            if (checkCharSizes(mr)) { // 验证尺寸，主要验证高度是否满足要求，去掉不符合规格的字符，中文字符后续处理
                charRect.add(mr);
                if (debug) {
                    ImageUtil.drawRectangle(dst, mr, debug, tempPath);
                }
            }
        }
