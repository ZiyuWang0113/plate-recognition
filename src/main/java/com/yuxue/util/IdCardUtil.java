...
public class IdCardUtil {
    ...
    private static final String TEMP_PATH = Constant.BASE_DIR + "CardDetect/temp/";
    ...
    public static String recoChars(File file, Rect r) {
        ...
        try {
            ...
            result = result.replaceAll("%", "X").replaceAll(" ", "").replaceAll("\n", "");
    ...
    public static Mat cardDetect(Mat src, Boolean debug, String tempPath) {
        ...
        if(null == face || face.width <= 0){
            log.error("未检测到人脸，非身份证！！");
            return null;
        }
    ...
    public static void txtDetect(Mat card, Boolean debug, String tempPath){
        ...
        Mat grey = new Mat();
        String greyFile = ImageUtil.gray(gsMat, grey, debug, tempPath);
        ...
        Rect idNumRect = getIdNumRect(threshold);
        ImageUtil.drawRectangle(grey, idNumRect, debug, tempPath); // 描绘识别的区域

        File flie = new File(greyFile);

        // 定向识别文字 // 使用Tess文字识别库，目前未经过算法优化处理，准确率较低
        String result = recoChars(flie, idNumRect);
     ...
     * @return 矩形框 灰度图区域
     */
    public static Rect getIdNumRect(Mat threshold){
        int minRow = 0;
        int maxRow = threshold.rows();
        int minCols = 0;
        int maxCols = threshold.cols();

        for (int i = threshold.rows() - 1; i > threshold.rows() / 2; i--) {
            Mat row = threshold.row(i).colRange(0, threshold.cols());
            int count = Core.countNonZero(row);
            if(maxRow == threshold.rows() && count > 200){
                maxRow = i;
            }
            if(maxRow != threshold.rows() && count < 100){
                minRow = i;
                break;
            }
        }
        ...
        int c = 0;
        for (int i = threshold.cols() - 1; i > 0; i--) {
            Mat col = threshold.col(i).rowRange(minRow, maxRow);
            int count = Core.countNonZero(col);
            if(maxCols == threshold.cols() && count > 10){
                maxCols = i;
            }
            if(count > 10){
                c = 0;
            ...
            if(maxCols != threshold.cols() && c > 10){
                minCols = i + 10;
                break;
            }
        }

        Rect result = new Rect(minCols, minRow, maxCols-minCols, maxRow-minRow);
        return result;
    }
    ...
    public static void main(String[] args) {
        ...
        Mat src = ImageUtil.imread(Constant.BASE_DIR + "CardDetect/test/4.jpg", CvType.CV_8UC3);
        Mat card = cardDetect(src, debug, tempPath); // 检测身份证

        // 识别身份证上的文字
        // Mat card = ImageUtil.imread(Constant.BASE_DIR  + "CardDetect/test/3.jpg", CvType.CV_8UC3);
