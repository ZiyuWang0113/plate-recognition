...
public class ImageUtil {
     ...
     * @param inMat
     */
    public static String debugImg(Boolean debug, String tempPath, String methodName, Mat inMat) {
        if (debug) {
            // 通过getId生成文件名称，使得每个步骤生成的图片能够按照执行时间进行排序
            String dir = tempPath + GenerateIdUtil.getStrId() +"_" +methodName + ".jpg";
            Imgcodecs.imwrite(dir, inMat);
            return dir;
        }
        return null;
    }
     ...
     * @return greyMat
     */
    public static String gray(Mat inMat, Mat dst, Boolean debug, String tempPath) {
        Imgproc.cvtColor(inMat, dst, Imgproc.COLOR_BGR2GRAY);
        String greyFile = debugImg(debug, tempPath, "gray", dst);
        return greyFile;
    }
     ...
     * @param rect 正矩形
     */
    public static void drawRectangle(Mat inMat, Rect rect, Boolean debug, String tempPath) {
        Imgproc.rectangle(inMat, new Point(rect.x, rect.y), new Point(rect.x + rect.width, rect.y + rect.height), new Scalar(0, 255, 0));
        if(debug){
            debugImg(debug, tempPath, "drawRectangle", inMat);
        }
    }
